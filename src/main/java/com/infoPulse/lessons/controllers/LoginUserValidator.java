...
//        boolean isUserExists = daoUser.isEntityExist(User.class, ((User) o).getLogin());
        ...
        if (!isUserExists) {
            errors.rejectValue("login", "error.loginNotExists");
            return;
        }
//        User userDB = daoUser.findEntityForID(User.class, ((User) o).getLogin());
        User userDB = daoUser.findUserByLogin(((User) o).getLogin());
        if (!userDB.getPassword().equals(((User) o).getPassword())) {
            errors.rejectValue("password", "error.passwordNotEquals");
            return;
        }
